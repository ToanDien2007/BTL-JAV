...
function injectOrderStyles() {
  ...
  s.textContent = `
    #oh-modal *, #oh-modal *::before, #oh-modal *::after {
  box-sizing: border-box;
    ...
    #oh-body::-webkit-scrollbar       { width: 5px; }
    #oh-body::-webkit-scrollbar-track { background: transparent; }
    #oh-body::-webkit-scrollbar-thumb {
      background: #e0e0e0;
      border-radius: 99px;
    }
    #oh-body::-webkit-scrollbar-thumb:hover { background: #ff3d7f; }
  ...
  flex-shrink: 0; /* Không cho phép card bị ép bẹp */
}
    .oh-card-head {
      display: flex; align-items: center; justify-content: space-between;
      padding: 12px 16px;
  ...
  gap: 12px; 
  align-items: center; 
  min-height: 52px; /* Đảm bảo đủ khoảng trống cho ảnh và chữ */
  padding: 4px 0;
}
    .oh-item-img {
...
async function openOrderHistory() {
  ...
  try {
    ...
    document.getElementById("oh-body").innerHTML =
      '<div class="oh-loading" style="color:#e53935">Lỗi tải dữ liệu.</div>';
  }
}
        ...
        async () => {
          ...
          try {
            ...
            if (r.ok) {
              ...
              if (card) {
                card.querySelector(".oh-status").className =
                  "oh-status status-cancelled";
...
if (window.location.pathname.includes("login.html")) {
  ...
  errMsg.style.cssText =
    "color:#e53935;text-align:center;margin:8px 0 0;font-size:14px;";
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("/api/login", {
        ...
        credentials: "include",
        body: JSON.stringify({
          identifier: inputs[0].value.trim(),
...
if (window.location.pathname.includes("forgot-password.html")) {
  ...
  if (btnSendOtp) {
    btnSendOtp.addEventListener("click", async (e) => {
      ...
      if (!email)
        return window.showToast?.("Vui lòng nhập email trước.", "warn");
